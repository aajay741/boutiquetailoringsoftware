...
try {
    ...
    // Add master_id filter if provided
    if (!empty($master_id)) {
        $where[] = "a.master_id = ?";
        $params[] = $master_id;
        $types .= 'i';
    ...
    $sql .= " GROUP BY p.purchase_id, p.invoice_number, s.store_name, c.name, c.phone, 
              ...
              u.first_name, u.last_name, a.master_id";
    ...
    // Add sorting by out_date (NULLs last) and pagination to main query
    $sql .= " ORDER BY a.out_date IS NULL, a.out_date DESC LIMIT ? OFFSET ?";
    $params[] = $limit;
    $params[] = $offset;
    $types .= 'ii';
